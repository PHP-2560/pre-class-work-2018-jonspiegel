...
#not sure where this is faiiling. Idea is select a specific row or column at a time and then pick out the values that give you issue
```
...
```{r}
  ...
  sum6 = 5
  for(2:6)
     if (survey[1,i]<1 || survey[1,i]>5) {
       sum1 = sum1 - 1
     }
  print(sum1)
  
  #not sure why this isn't working, once it works need to repeat it for each partic and then make that into a vector and then add that vector into the dataset
