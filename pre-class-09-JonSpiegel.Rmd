...
html_table(read_html(savedurl, "table")[[2]]) #this is how he does it in the videos.. not sure why it wont let me pick the second table??? It does let me here get the tables (if I remove the [[2]])
j <- read_html(savedurl, "table")
html_table(j)
```
4. Extract all of the "3 Month % Change" values from the "Stock Sectors" table.
```{r}
#however you read the first column in part 3, its the same table, next column
```
