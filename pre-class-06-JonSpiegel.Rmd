...
library(dplyr)
starwars
sd.of.numerics <- function(x) {
  if(is.numeric(x)) {
    sd(x)
  }
  else(return('this is not a numeric'))
}

vapply(starwars, is.numeric, T)
vapply(starwars, sd.of.numerics, 1)




for(i in ncol(starwars)) {
  
  if(is.numeric(starwars[,i])) {
    <- sd(starwars[,i])
  } #can hack part 2, will need to initialize a vector, add in an else condition in which we make NA values then remove those
}
