...
```{r}
standardizedwcgs <- for (i in 1:ncol(WCGS.new)) {
  standardize.me(WCGS.new[,i]) 
}#whatever else the function eneds as input may change. This should go col by col and standardize it
standardizedwcgs
standardize.me(WCGS.new[,1]) #so its working for an individual column but not in my for-loop right now
